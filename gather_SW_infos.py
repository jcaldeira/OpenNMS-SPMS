...
def validate_date_inter(date):
    ...
    high_date_lim = datetime.date(2021, 7, 8)
...
def env_exec():
    with open(os.path.join('Outputs', 'output-SW-info.txt'), 'w') as f:  # clear output file
        pass
...
def write_output(equipment, model, serial_number, ios, hostname):
    with open(os.path.join('Outputs', 'output-SW-info.txt'), 'at') as f:
        f.write(
            f"{equipment['secret']} ({equipment['ip']}):\t{model}\t{serial_number}\t{ios}\t{hostname}\n")
        main_logger.info(
            f"Completed: {equipment['secret']} ({equipment['ip']})")
...
if __name__ == '__main__':
    ...
    main_logger.info(
        f'Finished in {(perf_counter_stop - perf_counter_start):.3f} second(s)')
